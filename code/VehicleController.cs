...
public class WheelData
	{
	public Wheel wheel;                             // Wheel data from the inspector //检查员的车轮数据
        public WheelCollider collider;                  // WheelCollider component for this wheel车轮碰撞器组件
        public Transform transform;                     // Transform of the WheelCollider component   车轮碰撞器的位置
        public Vector3 origin;                          // Origin point cosidering WheelCollider.center车轮中心的起点？
        public float forceDistance; //预测距离
    public float steerAngle = 0.0f;//转向角
        public bool grounded = false;
	public WheelHit hit;                   //由WheelCollider报告的车轮的接触信息。
	public GroundMaterial groundMaterial = null;

	public float suspensionCompression = 0.0f;
	public float downforce = 0.0f;//向下作用力
	...
	public Vector2 tireSlip = Vector2.zero; //轮胎打滑
        public Vector2 tireForce = Vector2.zero;
	public Vector2 dragForce = Vector2.zero;

	public float angularVelocity = 0.0f;//角速度
        public float angularPosition = 0.0f;//    角位置
	...
	public RaycastHit rayHit;           //用于轮胎定位								// Result of raycast for visual wheels. Used for precise positioning (i.e. tire marks).

        // Extended tire data.
        // Calculated when extendedTireData is set to True by components.

        public float combinedTireSlip = 0.0f;       // Combined tire slip magnitude, in m/s   //组合轮胎滑移量
...
public class VehicleController : MonoBehaviour
	...
	[Range(0,2)]
	public float tireFriction = 1.0f;   //轮胎摩擦
    [Range(0,1)]
	public float antiRoll = 0.2f;    //翻滚
	[Range(0,89)]
	public float maxSteerAngle = 35.0f;   //最大转向角
     public float aeroDrag = 0.0f;  //空气阻力
        public float aeroDownforce = 1.0f;  //空气下降力
	...
	public float maxSpeedForward = 27.78f;//最大速度前进
        public float maxSpeedReverse = 12.0f;//最大速度倒转
        public float maxDriveForce = 2000.0f;//最大驱动力
        

        [Range(0.0001f, 0.9999f)]
	public float forceCurveShape = 0.5f;//预测形态
        public float maxDriveSlip = 4.0f;
	public float driveForceToMaxSlip = 1000.0f;//传动轴滑移

        [Header("Brakes")]

	public float maxBrakeForce = 3000.0f;//最大刹车力
        public float brakeForceToMaxSlip = 1000.0f;

	public enum BrakeMode { Slip, Ratio };  //声明枚举BrakeMode
        public BrakeMode brakeMode = BrakeMode.Slip;
	public float maxBrakeSlip = 2.0f;
	[Range(0,1)]
	public float maxBrakeRatio = 0.5f;//最大制动比

        public BrakeMode handbrakeMode = BrakeMode.Slip;
	public float maxHandbrakeSlip = 10.0f;//手制动模式
        [Range(0,1)]
	...
	public bool disallowRuntimeChanges = false;//不允许运行时更改
        ...
        //wheelhit校正固定车轮的接触点，这给移位以及速度。用一个额外的光线投射每轮每物理帧。
        ...
        public bool disableSteerAngleCorrection = false;//禁止转向角校正
        public bool showContactGizmos = false;   //显示接触图形

        [Header("Experimental")]
	[Range(0,0.5f)]
	public float sleepVelocity = 0.1f;//睡眠速度
        [Range(0,1)]
	public float steeringOverdrive = 0.0f;//驾驶超速

        // Ground physic properties applied when no proper ground material is available.
        public float defaultGroundGrip = 1.0f;//默认的地面控制
        public float defaultGroundDrag = 0.0f;//

        // Public non-exposed properties. To be configured from scripting if necessary.
        //公共非暴露属性。如有必要，通过脚本进行配置。
	...
	[NonSerialized] public float impactThreeshold = 0.6f;       // 0.0 - 1.0. The DotNormal of the impact is calculated. Less than this value means drag, more means impact.
        ...
        [NonSerialized] public bool computeExtendedTireData = false;// Components using extended tire data (tire marks, smoke, particles, audio) set this to True
                                                                    //计算加长轮胎数据              组件使用扩展轮胎数据(轮胎标记，烟雾，粒子，音频)设置为真
        ...
        //附加组件订阅此委托以获得影响通知。
        //使用FieldControlLo..Currar访问发送事件的控制器的值
...
//私有值在调用事件之前得到适当的映射。
        ...
        {
            get
            { return m_sumImpactPosition;
            }
        }//局部冲击位置
        public Vector3 localImpactVelocity { get { return m_sumImpactVelocity; } }//局部冲击速度
        public bool isHardImpact { get { return m_sumImpactHardness >= 0; } }

	// Body drag properties. Can be queued continuously.

	public Vector3 localDragPosition { get { return m_localDragPosition; } }//局部阻力位置
        public Vector3 localDragVelocity { get { return m_localDragVelocity; } }
	public bool isHardDrag { get { return m_localDragHardness >= 0; } }


        // Utility, also available for add-on components实用工具，也可用于附加组件
	...
	public float speedAngle { get { return m_speedAngle; } }
        ...
        // Cached and internal data, some accesible from scripting缓存的和内部的数据，一些脚本可以使用

        Transform m_transform;
	Rigidbody m_rigidbody;
	GroundMaterialManager m_groundMaterialManager;//：地面材料管理
	...
	CommonTools.BiasLerpContext m_forceBiasCtx = new CommonTools.BiasLerpContext();
        ...
        void OnValidate()
        {
            // Some parameters must be non-negative.某些参数必须是非负的。
        ...
        void OnEnable()
        {
            // Cache/find components and configure rigidbody缓存/查找组件和配置刚度
            ...
            m_rigidbody.maxDepenetrationVelocity = 8.0f;//依赖速度

            if (centerOfMass)
                m_rigidbody.centerOfMass = m_transform.InverseTransformPoint(centerOfMass.position);
            //Transform.InverseTransformPoint 相反变换位置   变换位置从世界坐标到自身坐标。和Transform.TransformPoint相反。
            if (wheels.Length == 0)
            {
        ...
        void UpdateSteering(WheelData wd)//转向更新
        {
            if (wd.wheel.steer)
            {
        ...
        float FixSteerAngle(WheelData wd, float inputSteerAngle)//修复引导角
        {
        ...
        void UpdateSuspension(WheelData wd)//悬浮更新
        {
            // Retrieve the wheel's contact point检索车轮的接触点
        ...
        void UpdateLocalFrame(WheelData wd)//更新本地框架/结构
        {
        ...
        void UpdateGroundMaterial(WheelData wd)//更新材料
        {
            if (wd.grounded)
        ...
        void ComputeTireForces(WheelData wd)//计算轮胎力
        {
        ...
        void ApplyTireForces(WheelData wd)//申请轮胎力
        {
            if (wd.grounded)
            {
                if (!disallowRuntimeChanges)
        ...
        static float ComputeSlipAngle(Vector2 localVelocity)//计算滑动角
        {
        ...
        static float ComputeCombinedSlip(Vector2 localVelocity, Vector2 tireSlip)//计算滑移
        {
        ...
        void ComputeExtendedTireData(WheelData wd, float referenceDownforce)//计算扩展轮胎数据
        {
        ...
        float ComputeDriveForce(float demandedForce, float maxForce, bool grounded)//计算驱动力量
        {
        ...
        void UpdateTransform(WheelData wd)               //更新位置
        {
        ...
        // Private methods for dealing with colliders处理碰撞器的私有方法

        Collider[] m_colliders = new Collider[0];
        int[] m_colLayers = new int[0];


        void FindColliders()//找到对撞机
        {
        ...
        void DisableCollidersRaycast()//禁用碰撞器射线投射
        {
        ...
        void EnableCollidersRaycast()//启用碰撞器射线投射
        {
            for (int i = 0, c = m_colliders.Length; i < c; i++)
        ...
        float GetWheelForceDistance(WheelCollider col)//得到车轮受力距离
        {
        ...
        void UpdateWheelCollider(WheelCollider col)//更新轮对撞
        {
        ...
        void SetupWheelCollider(WheelCollider col)//设置轮对撞机
        {
        ...
        void UpdateWheelSleep(WheelData wd)//更新轮睡眠
        {
        ...
        // Contact processing接触过程


        // Private data for internal use内部使用的私有数据
        ...
        void OnCollisionEnter(Collision collision)//碰撞输入
        {
        ...
        void OnCollisionStay(Collision collision)//碰撞停留
        {
            if (processContacts)
        ...
        void ProcessContacts(Collision col, bool forceImpact)//接触过程
        {
        ...
        // Impact processing处理碰撞

        void HandleImpacts()//处理碰撞
        {
        ...
        void UpdateDragState(Vector3 dragPosition, Vector3 dragVelocity, int dragHardness)//更新拖动状态
        {
        ...
        [ContextMenu("Adjust WheelColliders to their meshes")]//调整碰撞轮到他们的网格
        void AdjustWheelColliders()//调整WheelColliders
        {
            foreach (Wheel wheel in wheels)
            {
        ...
        static void AdjustColliderToWheelMesh(WheelCollider wheelCollider, Transform wheelTransform)//调整碰撞器到车轮网格
        {
        ...
        static Bounds GetScaledBounds(MeshFilter meshFilter)//得到了边界
        {
